...
| v3.0 | 2026-04-29 | 陈城 | 补充性能指标设计依据，细化日志监控设计，优化附录文档对照 | 待审核 |
...
| 接口类型 | 目标响应时间 | 95 分位响应时间 | 设计依据 |
|---------|-------------|----------------|----------|
| 普通接口（登录、列表查询） | ≤300ms | ≤500ms | 基于 FastAPI 异步框架性能（单进程可处理 1000+ QPS），结合本系统轻量化架构设计，确保高频操作无明显卡顿 |
| AI 接口（答疑、总结、出题） | ≤2s | ≤3s | 结合文心一言平均响应时间（2-3s），本系统采用多服务商降级策略，预留缓冲应对网络波动与服务商限流 |
| 文件上传（≤10MB） | ≤3s | ≤5s | 本系统采用本地文件存储，避免云存储网络延迟，结合断点续传机制保障大文件传输体验 |
| 页面首次加载 | ≤1.5s | ≤2s | 基于 Vue 3 + Vite 构建工具的性能优势，配合静态资源压缩与懒加载策略，满足 Google 核心 Web 指标要求 |
...
| 指标 | 目标值 | 设计依据 |
|------|-------|----------|
| 同时在线用户数 | ≥100 | 基于本系统面向高校课程班级场景（50-80人），结合轻量化架构设计预留25%缓冲容量应对突发集中访问 |
| 接口 QPS | ≥50 | 基于 FastAPI 异步框架性能（单进程可处理 1000+ QPS），本系统采用单进程部署，保守预留50%性能余量应对业务峰值 |
| AI 接口并发调用 | ≥20 | 结合文心一言并发调用限制（单账号≥30并发），本系统采用多服务商降级策略，确保多用户同时使用AI功能无阻塞 |
| 数据库连接数 | ≤50 | 匹配 MySQL 8.0 默认连接池配置上限，结合本系统业务量设计，避免连接耗尽导致服务不可用 |
...
| 指标 | 目标值 | 设计依据 |
|------|-------|----------|
| 系统可用性 | ≥99.5% | 基于阿里云ECS 2核4G服务器SLA（99.95%可用性），本系统采用轻量化架构减少单点故障，结合每日备份策略，确保年停机时间≤43.8小时，满足教学系统日常使用需求 |
| AI 接口调用成功率 | ≥98% | 结合文心一言API官方SLA（99.9%可用性）和通义千问API官方SLA（99.9%可用性），本系统采用多服务商降级策略+3次重试机制，理论上调用成功率可达99.9999%，保守设定为98% |
| 数据库无故障时间 | ≥30 天 | 基于MySQL 8.0社区版官方稳定性数据（平均无故障时间≥100天），本系统采用单数据库部署+每日凌晨自动备份策略，结合硬件服务器可靠性，设定为≥30天 |
...
| 指标 | 目标值 | 设计依据 |
|------|-------|----------|
| CPU 使用率（峰值） | ≤70% | 本系统采用FastAPI异步框架，CPU资源利用率高，预留30%CPU资源应对突发流量，避免系统过载导致响应延迟 |
| 内存占用（峰值） | ≤80% | 本系统采用轻量化架构设计，内存占用低，遵循Linux系统内存管理最佳实践，预留20%内存缓冲防止OOM错误 |
| 磁盘使用率 | ≤85% | 本系统采用本地文件存储，结合每日备份策略，预留15%磁盘空间应对数据增长与备份存储需求 |
...
| 日志级别 | 说明 | 应用层级 | 设计依据 |
|---------|------|----------|----------|
| DEBUG | 调试信息，记录函数调用、参数传递等细节 | 开发环境 | 用于开发调试，定位代码逻辑问题 |
| INFO | 普通操作信息，记录用户登录、接口调用等业务行为 | 生产环境 | 用于业务审计和流量分析 |
| WARN | 潜在风险警告，如接口超时、资源不足等 | 生产环境 | 提前发现潜在问题，避免系统故障 |
| ERROR | 错误信息，记录异常堆栈、错误原因 | 生产环境 | 用于故障排查，快速定位问题根源 |

**日志存储策略**：
- 开发环境：控制台输出
- 生产环境：文件存储（每日滚动）+ 日志聚合工具（如 ELK）
- 日志保留时间：180天
...
| 监控项 | 告警阈值 | 设计依据 |
|-------|---------|----------|
| CPU 使用率 | ≥80% | 本系统采用FastAPI异步框架，CPU资源利用率高，预留20%CPU资源应对突发流量，避免系统过载 |
| 内存使用率 | ≥85% | 本系统采用轻量化架构设计，内存占用低，参考Linux系统内存管理最佳实践，预留15%内存缓冲防止OOM错误 |
| 接口响应时间 | ≥3s | 超过普通接口目标响应时间10倍，结合本系统业务特性判定为性能异常 |
| 错误率 | ≥5% | 参考行业标准，结合本系统业务场景，错误率超过5%判定为服务异常 |
| AI接口成功率 | ≤95% | 低于稳定性指标要求（≥98%），触发本系统多服务商切换策略 |

**监控实现方式**：
- 系统监控：Prometheus + Grafana
- 应用监控：FastAPI内置metrics接口
- 告警方式：邮件+企业微信通知
...
| SDD 章节 | SRS 章节 | 差异说明 |
|---------|---------|----------|
| 2.1 系统总体架构 | 3.1 系统架构 | SDD 补充了架构特点和技术选型细节 |
| 3.1 CSCI-01 前端应用 | 4.1 学生端功能 | SDD 细化了前端组件和 API 调用 |
| 3.2 CSCI-02 后端应用 | 4.2 教师端功能 | SDD 补充了后端模块划分和接口设计 |
| 3.3 CSCI-03 AI 服务模块 | 4.3 AI 功能 | SDD 增加了多服务商切换和降级策略 |
| 5 性能指标设计 | 5.1 性能需求 | SDD 补充了性能指标的设计依据 |

#### SDD 与 spec.md v1.0 对照表

| SDD 章节 | spec.md 章节 | 差异说明 |
|---------|---------|----------|
| 4.1 接口设计 | 3.1 API 规范 | SDD 细化了接口参数和返回格式 |
| 7.2 可维护性设计 | 4.1 日志规范 | SDD 补充了日志级别和存储策略 |
| 7.3 安全性设计 | 4.2 安全规范 | SDD 增加了认证授权和数据安全细节 |
